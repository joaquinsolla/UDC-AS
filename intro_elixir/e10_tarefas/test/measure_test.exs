defmodule MeasureTest do
  ...
  doctest Measure
   ...
   property "ao medir N funcións créanse N*2 tarefas" do
     check all l <- pequena_lista_de_funcions(),
               n <- large_integer(), max_runs: 8 do
       ...
       assert length(tarefas_creadas) == length(l) * 2, "non se crearon o número de tarefas indicado"
       assert length(tarefas_rematadas_ou_interrompidas) == length(l) * 2, "non se eliminaron todas as tarefas creadas"
     end
   end
   ...
   defp lista_de_funcions() do
     ...
     nonempty(list_of(member_of([{Manipulating, :reverse},
                                 {Manipulating, :flatten},
                                 {Sorting, :quicksort},
                                 {Sorting, :mergesort}]))) do
       lista
     end
   end
